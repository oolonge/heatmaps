...
# Класс для создания тепловой карты близости образовательных программ
class EduProgramProximityMap:
    def __init__(self):
    ...
    def load_all_data(self):
        ...
        try:
            self.program_data = pd.read_csv('data/courses.csv', sep=';')
            self.program_names = self.program_data['Название'].tolist()
        except Exception as e:
            print(f"Ошибка при загрузке данных об образовательных программах: {e}")
            return False
        
        # Загружаем матрицу сходства университетов
        try:
        ...
        # Загружаем матрицу сходства экзаменов
        try:
    ...
    def preprocess_data(self):
        ...
        if numeric_columns:
            numeric_data = self.program_data[numeric_columns].copy()
            for col in numeric_columns:
                numeric_data[col] = pd.to_numeric(numeric_data[col], errors='coerce')
            
            scaler = MinMaxScaler()
            self.program_data[numeric_columns] = scaler.fit_transform(numeric_data)
    ...
    def calculate_similarity(self, program1, program2):
        # Извлекаем значения числовых признаков
        numeric_features1 = []
        numeric_features2 = []
        
        for feature, weight in self.features['числовые'].items():
            if feature in program1 and feature in program2:
                val1 = float(program1[feature]) * weight
                val2 = float(program2[feature]) * weight
        ...
        for feature, weight in self.features['категориальные'].items():
            if feature in self.similarity_matrices and feature in program1 and feature in program2:
                val1 = program1[feature]
                val2 = program2[feature]
    ...
    def calculate_proximity_matrix(self):
        ...
        n_programs = len(self.program_data)
        proximity_matrix = np.ones((n_programs, n_programs))
        
        # Для каждой пары образовательных программ
        for i in range(n_programs):
            for j in range(i + 1, n_programs):
                program1 = self.program_data.iloc[i]
                program2 = self.program_data.iloc[j]
                
                similarity = self.calculate_similarity(program1, program2)
    ...
    def create_heatmap(self, save_file=False):
        ...
        sns.set(font_scale=0.7)  # Уменьшаем размер шрифта для вмещения длинных названий
        ...
        ax = sns.heatmap(
            ...
            annot=True,
            cmap="YlGnBu",
            xticklabels=self.program_names,
            yticklabels=self.program_names,
            vmin=0,
            vmax=1,
            linewidths=0.5,
            fmt=".2f"
        ...
        plt.title(f"Тепловая карта {method_names[self.distance_method]} сходства образовательных программ в Китае", fontsize=16)
        ...
        if save_file:
            ...
            output_file = f"edu_heatmap_{method_names_file[self.distance_method]}.png"
...
def main():
    parser = argparse.ArgumentParser(description='Программа для создания тепловой карты близости образовательных программ в Китае')
    parser.add_argument('-a', '--all', action='store_true', help='Создать тепловые карты для всех методов расчета расстояния')
    args = parser.parse_args()
    
    proximity_map = EduProgramProximityMap()
    
    if not proximity_map.load_all_data():
        print("Ошибка при загрузке данных. Запустите сначала скрипт prepare_china_data.sh для создания необходимых файлов.")
        sys.exit(1)
