...
class CourseRecommenderGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Система рекомендации языковых курсов")
        ...
        liked_label = QLabel("Понравившиеся курсы:")
        ...
        disliked_label = QLabel("Не понравившиеся курсы:")
    ...
    def add_liked_course(self, course_name):
        ...
        if course_name in self.disliked_courses:
            QMessageBox.warning(self, "Предупреждение", 
                               f"Курс '{course_name}' уже добавлен в непонравившиеся курсы.")
            return
        
        # Check if the course is already in liked courses
        if course_name in self.liked_courses:
            QMessageBox.information(self, "Информация", 
                                  f"Курс '{course_name}' уже добавлен в понравившиеся курсы.")
            return
        ...
        if len(self.liked_courses) + len(self.disliked_courses) >= MAX_LIKED_DISLIKED_COURSES:
            QMessageBox.warning(self, "Предупреждение", 
                               f"Достигнуто максимальное количество выбранных курсов ({MAX_LIKED_DISLIKED_COURSES}).")
            return
    ...
    def search_courses(self):
        ...
        if not self.liked_courses and not self.disliked_courses:
            QMessageBox.warning(self, "Предупреждение", 
                               "Выберите хотя бы один понравившийся или непонравившийся курс.")
            return
    ...
    def __init__(self, course_info, original_data, parent=None):
        ...
        for key, value in course_info.items():
            if key != 'Название' and key != 'similarity_score':
                ...
                if key == 'Тип курса':
                    display_value = "Интенсивный" if int(float(value)) == 1 else "Обычный"
                elif key == 'Онлайн':
                    display_value = "Да" if int(float(value)) == 1 else "Нет"
                elif key in ['Цена', 'Кол-во часов', 'Уровень сложности']:
                    # Use original values from data
                    if original_course:
                        display_value = original_course[key]
                
                if key == 'Цена':
                    label_key = QLabel(f"Цена в час:")
...
class ResultsDialog(QDialog):
    ...
    def show_results_page(self):
        ...
        if not self.current_results:
            no_results = QLabel("Нет подходящих курсов")
...
def run_gui():
    ...
    if not recommender.load_all_data():
        QMessageBox.critical(None, "Ошибка", "Не удалось загрузить данные о курсах.")
        sys.exit(1)
