...
class CourseRecommender:
    ...
    def load_all_data(self):
        ...
        # Load product type similarity matrix
        try:
        ...
        # Load application area similarity matrix
        try:
        ...
        # Load years similarity matrix
        try:
            years_sim = pd.read_csv('data/years_similarity.csv', sep=';', index_col=0)
            # Convert indices to string for correct comparison
    ...
    def calculate_similarity(self, course1, course2):
        ...
        for feature, weight in self.features['категориальные'].items():
            if feature in self.similarity_matrices and feature in course1 and feature in course2:
                val1 = str(course1[feature])
                val2 = str(course2[feature])
    ...
    def get_recommendations(self, liked_indices, disliked_indices, top_n=None):
        ...
        Args:
            liked_indices: List of indices of liked products
            disliked_indices: List of indices of disliked products
            top_n: Number of recommendations to return
            
        Returns:
            List of dictionaries with product info and combined similarity score
        """
        if top_n is None:
