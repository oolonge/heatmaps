...
# Constants
MAX_LIKED_DISLIKED_COURSES = 10  # Maximum number of products that can be liked/disliked
...
class CourseRecommenderGUI(QMainWindow):
    def __init__(self, recommender):
        ...
        self.original_data = pd.read_csv('data/cosmetics.csv', sep=';')
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Система рекомендации косметических продуктов")
        ...
        liked_label = QLabel("Понравившиеся продукты:")
        ...
        disliked_label = QLabel("Не понравившиеся продукты:")
    ...
    def add_liked_course(self, course_name):
        ...
        if course_name in self.disliked_courses:
            QMessageBox.warning(self, "Предупреждение", 
                               f"Продукт '{course_name}' уже добавлен в непонравившиеся продукты.")
            return
        
        # Check if the course is already in liked courses
        if course_name in self.liked_courses:
            QMessageBox.information(self, "Информация", 
                                  f"Продукт '{course_name}' уже добавлен в понравившиеся продукты.")
            return
        ...
        if len(self.liked_courses) + len(self.disliked_courses) >= MAX_LIKED_DISLIKED_COURSES:
            QMessageBox.warning(self, "Предупреждение", 
                               f"Достигнуто максимальное количество выбранных продуктов ({MAX_LIKED_DISLIKED_COURSES}).")
            return
    ...
    def search_courses(self):
        ...
        if not self.liked_courses and not self.disliked_courses:
            QMessageBox.warning(self, "Предупреждение", 
                               "Выберите хотя бы один понравившийся или непонравившийся продукт.")
            return
    ...
    def __init__(self, course_info, original_data, parent=None):
        ...
        for key, value in course_info.items():
            if key != 'Название' and key != 'similarity_score':
                ...
                if key in ['объем, мл', 'цена', 'год презентации', 'аллергены']:
                    # Use original values from data
                    if original_course:
                        display_value = original_course[key]
                
                if key == 'цена':
                    label_key = QLabel(f"Цена:")
                    label_value = QLabel(str(display_value) + " ₽")
                elif key == 'объем, мл':
                    label_key = QLabel(f"Объем:")
                    label_value = QLabel(str(display_value) + " мл")
                elif key == 'аллергены':
                    label_key = QLabel(f"Аллергены:")
...
class ResultsDialog(QDialog):
    ...
    def __init__(self, results, original_data, parent=None):
        ...
        # Results title
        results_label = QLabel("Рекомендуемые продукты:")
    ...
    def show_results_page(self):
        ...
        if not self.current_results:
            no_results = QLabel("Нет подходящих продуктов")
...
def run_gui():
    from cosmetics_recommender import CourseRecommender  # Import only when needed
    ...
    if not recommender.load_all_data():
        QMessageBox.critical(None, "Ошибка", "Не удалось загрузить данные о косметических продуктах.")
        sys.exit(1)
